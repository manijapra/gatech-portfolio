...
public class sensor_server_udp
{
    ...
    static class MultiClientHelper implements Runnable
    {
        ...
        public MultiClientHelper(DatagramSocket socket, DatagramPacket packet)
        {
            clientSocket = socket;
            clientPacket = packet;
        }
        ...
        public void run()
        {
            try
            {
                ...
                {
                    System.out.println("\nClient Connected!");
                    System.out.println("Client Address: " + clientPacket.getAddress().getHostAddress());
                    System.out.println("Client Port: " + clientSocket.getPort());
                }

                //-a_req
                byteBuf = clientPacket.getData();
                ...
                clientPacket.setData(returnBuf);
                clientSocket.send(clientPacket);

                byteBuf = new byte[1024];
                clientPacket.setData(byteBuf);
                clientPacket.setLength(byteBuf.length);
    ...
    public static void main(String[] args) throws IOException
    {
        if (args.length > 0) //If arguments are available
        {
            ...
            try
            {
		        ...
		        DatagramPacket p = new DatagramPacket(byteBuf, byteBuf.length);

                while (true)
                {
                    s.receive(p);
                    MultiClientHelper miniServer = new MultiClientHelper(s, p);
        ...
        else
        {
            ...
            if (hashCheck) //Completes authentication check
            {
                if (sensorValidation.containsKey(tempUser))
                {
                    ...
                    {
                        System.out.println("Something went wrong while attempting to authenticate client.");
                    }
                }
                else
                {
