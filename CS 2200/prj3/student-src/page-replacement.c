...
pfn_t get_free_frame(void) {
   ...
   /* Note: Think of what kinds of frames can you return before you decide
	...
	while (infinite)
	{
		pcb_t *possibleEvictionPCB = rlt[i].pcb;
		vpn_t possibleEvictionVPN = rlt[i].vpn;

		if(possibleEvictionPCB != NULL)
		{
			if (possibleEvictionPCB->pagetable[possibleEvictionVPN].used)
			{
				possibleEvictionPCB->pagetable[possibleEvictionVPN].used = 0;
			}
			else
			{
				return i;
