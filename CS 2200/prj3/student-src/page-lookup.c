...
pfn_t pagetable_lookup(vpn_t vpn, int write) {
   pfn_t pfn = current_pagetable[vpn].pfn; //Page Table indexed with vpn
    ...
    */
	...
	{
		count_pagefaults++; //Increment Number of Page Faults
		return pagefault_handler(vpn, write); //Return the result of the handler
	}
